...
test('release metadata is aligned to v0.0.7', () => {
  ...
  assert.match(index, /3D City Response Simulator — v0\.0\.7/);
  assert.match(index, /validation\.js\?v=0\.0\.7/);
  assert.match(index, /app\.js\?v=0\.0\.7/);
  assert.match(app, /app-v0\.0\.7\.js\?v=0\.0\.7/);
});
