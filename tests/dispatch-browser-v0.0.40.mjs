// Dispatch E2E fixture: exercises the real isolated v0.0.40 module chain while Google/Cesium rendering is fixture-backed.
...
await new Promise(r=>server.listen(0,'127.0.0.1',r));const url=`http://127.0.0.1:${server.address().port}/dispatch-prototype-v0.0.40.html`;
...
async function setup(viewport){const context=await browser.newContext({viewport,hasTouch:viewport.width<600});const page=await context.newPage();const errors=[];page.on('pageerror',e=>{errors.push(e.message);console.error('PAGEERROR',e.message);});page.on('console',m=>{if(m.type()==='error')console.error('CONSOLE',m.text());});page.on('requestfailed',r=>console.error('REQUESTFAILED',r.url(),r.failure()?.errorText));await page.route('**/app-v0.0.8.js?v=0.0.8',async route=>{const response=await route.fetch();await route.fulfill({response,body:`(${fixture.toString()})();\n${await response.text()}`});});await page.route('https://tile.openstreetmap.org/**',r=>r.fulfill({status:200,contentType:'image/png',body:Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2nXQAAAAASUVORK5CYII=','base64')}));await page.goto(url);try{await page.waitForFunction(()=>window.__CITY_DEMO_RUNTIME__&&window.__DISPATCH_MODE_RUNTIME__&&window.__DISPATCH_GAMEPLAY__&&window.__DISPATCH_EMS_TRANSPORT__&&window.__DISPATCH_TABLET__&&window.__DISPATCH_AFTER_ACTION__,null,{timeout:15000});}catch(e){console.error('RUNTIME_GLOBALS',await page.evaluate(()=>({city:!!window.__CITY_DEMO_RUNTIME__,mode:!!window.__DISPATCH_MODE_RUNTIME__,game:!!window.__DISPATCH_GAMEPLAY__,ems:!!window.__DISPATCH_EMS_TRANSPORT__,tablet:!!window.__DISPATCH_TABLET__,review:!!window.__DISPATCH_AFTER_ACTION__,body:document.body.innerText.slice(0,300)})));throw e;}await page.locator('#api-key').fill('fixture-not-a-real-key');await page.locator('#launch-button').click();await page.waitForFunction(()=>window.__CITY_DEMO_RUNTIME__.snapshot().pickerActive);return {context,page,errors};}
...
// Exercise every authoritative base through the real UI in a fresh context so
 // state from one base cannot make another appear to pass.
 await main.context.close();
 for(const [service,count] of [['fire',3],['ems',2]]) for(let i=0;i<count;i++){
   const run=await setup({width:1440,height:900});
   const s=await launchBase(run.page,service,i);
   assert.equal(s.mode.service,service);
   assert.ok(Number.isFinite(s.vehicle.lat)&&Number.isFinite(s.vehicle.lon));
   const base=s.mode.base;
   assert.ok(Math.abs(s.vehicle.lat-Number(base.spawnLat))<0.00001);
   assert.ok(Math.abs(s.vehicle.lon-Number(base.spawnLng))<0.00001);
   assert.deepEqual(run.errors,[]);
   await run.context.close();
 }
 const main=await setup({width:1440,height:900}); const page=main.page; const errors=main.errors;
 await launchBase(page,'fire',0);
