import {
  ...
  advanceVehicle,
  roadAllowsFullSpeed,
} from './logic.js?v=0.0.7';
import { SurfaceAlignmentService } from './surface.js?v=0.0.7';
...
function setupUi() {
  ...
  const savedCity = readSetting('google3d.ontarioCity'); if (savedCity) dom.cityInput.value = savedCity;
  const savedSpawnAddress = readSetting('google3d.spawnAddress'); if (savedSpawnAddress && dom.spawnAddress) dom.spawnAddress.value = savedSpawnAddress;
  ...
  dom.spawnAddressButton?.addEventListener('click', spawnAtAddress);
  dom.recoverSurfaceButton?.addEventListener('click', recoverTruckToSurface);
...
window.__CITY_DEMO_RUNTIME__ = Object.freeze({ snapshot: () => ({
  ...
  fps: state.fps, input: getDriveInput(), locationName: state.station.name, surfaceHeight: state.surface.displayHeight,
}) });
