...
async function sampleGroundHeightAroundSpawn(lat,lon) {
  ...
  // The Google photogrammetry height at the exact station coordinate can be the roof.
  // Sample a ring outside the building footprint instead, then use a robust low-ground
  // estimate. The truck can remain at the authoritative X/Y inside the hall while its
  // Z comes from the surrounding terrain/yard rather than the roof mesh.
  const samples=[];
  for(const radius of [14,22,30]) for(let i=0;i<12;i++){
  ...
  const excluded=state.truckEntity?[state.truckEntity]:[];
  let heights=samples.map(p=>scene.sampleHeight(p,excluded)).filter(Number.isFinite);
  if(heights.length<8&&typeof scene.sampleHeightMostDetailed==='function'){
    try{const detailed=await scene.sampleHeightMostDetailed(samples,excluded);heights=detailed.map(p=>p?.height).filter(Number.isFinite);}catch{}
  }
  if(!heights.length) return null;
  heights.sort((a,b)=>a-b);
  // Median of the lowest third rejects roofs/trees while avoiding a single low outlier.
  const groundBand=heights.slice(0,Math.max(3,Math.ceil(heights.length/3)));
...
async function dispatchSpawnAtCoordinates(spawn) {
  ...
  if(Number.isFinite(groundHeight)){
    surfaceAlignment.reset();
    surfaceAlignment.accept(groundHeight,state.vehicle);surfaceAlignment.accept(groundHeight,state.vehicle);
    state.surface.targetHeight=groundHeight+0.15;state.surface.displayHeight=groundHeight+0.15;
    state.surface.locked=true;state.surface.mostDetailedTried=true;state.surface.lastAcceptedAt=performance.now();
  } else {
    refreshSurfaceHeight();
  }
  updateTruckEntity(); recenterCamera();
  window.dispatchEvent(new CustomEvent('city-dispatch-spawned',{detail:{...state.station,groundHeight}}));
