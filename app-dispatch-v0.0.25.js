...
await import('./app-dispatch-mode-v0.0.25.js?v=0.0.25');
...
window.__BOOT_RUN_LOG__?.('APP MODULE: app-dispatch-v0.0.25 reached');
const runBox=document.createElement('aside');runBox.id='run-diagnostics-box';runBox.innerHTML='<div class="run-head"><strong>RUN DIAGNOSTICS</strong><button id="run-copy" type="button">COPY</button></div><pre id="run-log">BOOT: page loaded\n</pre>';
document.body.append(runBox);
...
runLog('MODULES: dispatch application loaded');
...
window.addEventListener('city-dispatch-spawned',e=>runLog('SPAWN OK: '+JSON.stringify(e.detail)));
