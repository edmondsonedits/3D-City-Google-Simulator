...
async function resolveCityFireHall(apiKey, city) {
  ...
  for (const query of queries) {
    try {
      const result = await geocodeOntarioAddress(apiKey, query);
      // Address geocodes can land on the building footprint. Ask Google for the nearest
      // drivable road point and use that as the vehicle spawn while retaining the hall identity.
      await loadGoogleMapsGeocoder(apiKey);
      const roads = new window.google.maps.DirectionsService();
      const roadPoint = await new Promise(resolve => {
        roads.route({
          origin: { lat: result.lat, lng: result.lon },
          destination: { lat: result.lat + 0.0007, lng: result.lon + 0.0007 },
          travelMode: window.google.maps.TravelMode.DRIVING
        }, (response, status) => {
          if (status !== 'OK' || !response?.routes?.[0]?.legs?.[0]) return resolve(null);
          const leg=response.routes[0].legs[0], step=leg.steps?.[0];
          const p=leg.start_location || step?.start_location;
          if(!p) return resolve(null);
          let headingDeg=0;
          const q=step?.end_location;
          if(q){
            const lat1=p.lat()*Math.PI/180,lat2=q.lat()*Math.PI/180,dLon=(q.lng()-p.lng())*Math.PI/180;
            headingDeg=(Math.atan2(Math.sin(dLon)*Math.cos(lat2),Math.cos(lat1)*Math.sin(lat2)-Math.sin(lat1)*Math.cos(lat2)*Math.cos(dLon))*180/Math.PI+360)%360;
          }
          resolve({lat:p.lat(),lon:p.lng(),headingDeg});
        });
      });
      return { ...result, ...(roadPoint||{}), id:'free-fire-hall', name:`${city.name} · Main Fire Hall`, headingDeg:roadPoint?.headingDeg||0 };
    } catch {}
  }
  throw new Error(`Could not locate the main fire hall for ${city.name}.`);
}
