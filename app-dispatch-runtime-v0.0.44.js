...
const CAMERA_PRESETS = Object.freeze({
  ...
  birdsEye: { id: 'birdsEye', label: 'Bird’s-eye · GTA', range: 95, pitch: -1.08, targetHeight: 4.0, lookAhead: 13.0 },
});
...
const dom = {
  ...
  compass: document.querySelector('#live-compass'),
...
const state = {
  ...
  camera: { mode: 'FOLLOW', preset: 'birdsEye', yawOffset: 0, pitch: CAMERA_PRESETS.birdsEye.pitch, range: CAMERA_PRESETS.birdsEye.range },
...
function currentPreset() { return CAMERA_PRESETS[state.camera.preset] || CAMERA_PRESETS.birdsEye; }
function applyCameraPreset(id, { announce = true } = {}) {
  const preset = CAMERA_PRESETS[id] || CAMERA_PRESETS.birdsEye;
...
function resetVehicle({ forceSurface = true } = {}) {
  ...
  const fixedGroundHeight = Number(spawn?.groundHeight);
  state.dispatchGroundLock = Number.isFinite(fixedGroundHeight)
    ? { active: true, lat: spawn.lat, lon: spawn.lon, height: fixedGroundHeight }
    : null;
  ...
  if (forceSurface) {
    ...
    if (state.dispatchGroundLock) {
      surfaceAlignment.accept(fixedGroundHeight, state.vehicle);
      surfaceAlignment.accept(fixedGroundHeight, state.vehicle);
      const displayHeight = fixedGroundHeight + 0.15;
      state.surface.targetHeight = displayHeight;
      state.surface.displayHeight = displayHeight;
...
async function alignTruckToSurface({quiet=false}={}) {
  if (!state.demoRunning || !state.viewer) return false;
  const lockedHeight = currentDispatchGroundLockHeight();
  if (Number.isFinite(lockedHeight)) {
    applyKnownGroundHeight(lockedHeight, { snap: true });
    updateTruckEntity();
    recenterCamera();
    if (!quiet) setStatus(dom.surfaceStatus, 'Surface: station ground lock', 'good');
    return true;
  }
  const scene=state.viewer.scene;if(!scene?.sampleHeightSupported)return false;
  try{clearDriveInput();state.vehicle.speed=0;state.vehicle.steering=0;state.surfaceGeneration+=1;state.surface.pending=false;const cartographic=Cesium.Cartographic.fromDegrees(state.vehicle.lon,state.vehicle.lat,0);const excluded=state.truckEntity?[state.truckEntity]:[];let height=scene.sampleHeight(cartographic,excluded);if(!Number.isFinite(height)&&typeof scene.sampleHeightMostDetailed==='function'){const sampled=await scene.sampleHeightMostDetailed([cartographic],excluded);height=sampled?.[0]?.height;}if(!Number.isFinite(height))return false;surfaceAlignment.reset();surfaceAlignment.accept(height,state.vehicle);surfaceAlignment.accept(height,state.vehicle);const recoveredHeight=height+0.35;state.surface.targetHeight=recoveredHeight;state.surface.displayHeight=recoveredHeight;state.surface.locked=true;state.surface.mostDetailedTried=true;state.surface.lastAcceptedAt=performance.now();updateTruckEntity();recenterCamera();if(!quiet)setStatus(dom.surfaceStatus,'Surface: truck aligned','good');return true;}catch(e){if(!quiet)console.warn('Surface alignment failed',e);return false;}
}
async function centerAndAlign(){recenterCamera();await alignTruckToSurface();}
...
async function refreshSurfaceHeight() {
  ...
  const lockedHeight = currentDispatchGroundLockHeight();
  if (Number.isFinite(lockedHeight)) {
    applyKnownGroundHeight(lockedHeight);
    return;
  }
...
function compassDirection(heading){const deg=((heading*180/Math.PI)%360+360)%360;const labels=['N','NE','E','SE','S','SW','W','NW'];return {label:labels[Math.round(deg/45)%8],deg};}
function updateHud(driveInput) {
  const onRoad = isOnRoad(), preset = currentPreset();
  if(dom.compass){const d=compassDirection(state.vehicle.heading);dom.compass.textContent=d.label;dom.compass.title=Math.round(d.deg)+'°';}
  ...
  if (state.dispatchGroundLock?.active) setStatus(dom.surfaceStatus, `Surface: station ground ${state.surface.targetHeight.toFixed(1)} m`, 'good');
  else if (state.surface.locked) setStatus(dom.surfaceStatus, `Surface: locked ${state.surface.targetHeight.toFixed(1)} m`, surfaceAge < 2 ? 'good' : 'warn');
...
function moveTurboPosition(previous, next, dt) {
  if (!turboEnabled || next.speed <= TRUCK.maxForwardSpeed) return { lat:next.lat, lon:next.lon };
  const distance = next.speed * dt, north=Math.cos(next.heading)*distance, east=Math.sin(next.heading)*distance;
  const metersPerDegLat=111320, metersPerDegLon=Math.max(10000,metersPerDegLat*Math.cos(previous.lat*Math.PI/180));
  return {lat:previous.lat+north/metersPerDegLat,lon:previous.lon+east/metersPerDegLon};
}
function tick(now) {
  ...
  if (state.demoRunning && state.viewer && dom.setupOverlay.hidden && !document.hidden) {
    const driveInput = getDriveInput(); let next = advanceVehicle(state.vehicle, driveInput, dt, isOnRoad());
    if (turboEnabled && driveInput.brake < 0.01) {
      const baseCap = TRUCK.maxForwardSpeed;
      const boostedSpeed = Math.max(next.speed, state.vehicle.speed, baseCap) + TURBO_ACCEL * dt;
      next = { ...next, speed: boostedSpeed };
      const moved = moveTurboPosition(state.vehicle, next, dt);
      next.lat = moved.lat; next.lon = moved.lon;
    }
...
async function openOntarioPicker(apiKey, quality, cameraPreset) {
  ...
  // Show the picker UI first. Cesium map creation is optional: on mobile/browser failures,
  // the player must still be able to continue into Peterborough dispatch.
  dom.setupOverlay.hidden = true;
  dom.cityPickerOverlay.hidden = false;
  document.body.classList.add('city-picker-active');
  dom.cityPickerName.textContent = ONTARIO_CITIES[picker.selectedIndex].name;
  dom.cityPickerStart.textContent = 'Start in ' + ONTARIO_CITIES[picker.selectedIndex].name;
  try {
    picker.viewer = createOntarioPickerViewer();
    setPickerCity(picker.selectedIndex, 'open');
  } catch (error) {
    console.error('[City picker] Visual map unavailable; continuing with selected city.', error);
    picker.viewer = null;
    dom.cityPickerHint.textContent = 'Map preview unavailable · Peterborough is selected';
  }
}

const VERIFIED_FIRE_HALL_ADDRESSES = Object.freeze({
  'Barrie':'155 Dunlop St W, Barrie, ON',
  'Belleville':'60 Bettes St, Belleville, ON',
  'Brampton':'25 Rutherford Rd S, Brampton, ON',
  'Brockville':'360 Laurier Blvd, Brockville, ON',
  'Burlington':'1255 Fairview St, Burlington, ON',
  'Cambridge':'1625 Bishop St N, Cambridge, ON',
  'Cornwall':'705 Tollgate Rd W, Cornwall, ON',
  'Hamilton':'35 John St N, Hamilton, ON',
  'Kitchener':'270 Strasburg Rd, Kitchener, ON',
  'North Bay':'119 Princess St W, North Bay, ON',
  'Ottawa':'1445 Carling Ave, Ottawa, ON',
  'Peterborough':'210 Sherbrooke St, Peterborough, ON',
  'Kingston':"500 O'Connor Dr, Kingston, ON"
});

const VERIFIED_ROAD_SPAWNS = Object.freeze({
  // Station 1's address geocode is the building. This point is deliberately on
  // Sherbrooke Street in front of the hall, so Free Drive never starts on the roof.
  'Peterborough': { lat:44.3007165, lon:-78.3217746, headingDeg:91.7, road:'Sherbrooke Street' }
});

async function resolveCityFireHall(apiKey, city) {
  const verified = VERIFIED_FIRE_HALL_ADDRESSES[city.name];
  const queries = verified ? [verified] : [
    `${city.name} Fire Department Headquarters, Ontario, Canada`,
    `${city.name} Fire Station 1, Ontario, Canada`,
    `${city.name} Fire Department, Ontario, Canada`
  ];
  for (const query of queries) {
    try {
      const hall = await geocodeOntarioAddress(apiKey, query);
      const fixedRoad = VERIFIED_ROAD_SPAWNS[city.name];
      if (fixedRoad) return { ...hall, ...fixedRoad, id:'free-fire-hall-road', name:`${city.name} · Main Fire Hall` };

      // For other cities, snap the hall to Google's nearest road coordinate. Never
      // fall back to the building coordinate if road snapping fails.
      await loadGoogleMapsGeocoder(apiKey);
      const geocoder = new window.google.maps.Geocoder();
      const roadResults = await new Promise((resolve,reject)=>{
        geocoder.geocode({location:{lat:hall.lat,lng:hall.lon}},(items,status)=>{
          if(status==='OK'&&items?.length) resolve(items); else reject(new Error('No road geocode'));
        });
      });
      const roadResult=roadResults.find(item=>item.types?.includes('route')||item.address_components?.some(x=>x.types?.includes('route')));
      const p=roadResult?.geometry?.location;
      if(!p) throw new Error('No drivable road found beside fire hall');
      return { ...hall, lat:p.lat(), lon:p.lng(), id:'free-fire-hall-road', name:`${city.name} · Main Fire Hall`, headingDeg:0 };
    } catch {}
  }
  throw new Error(`Could not locate a road spawn beside the main fire hall for ${city.name}.`);
}

async function launchSelectedCity() {
  if (picker.launching) return;
  ...
  dom.cityPickerHint.textContent = 'Finding ' + city.name + ' main fire hall…';
  ...
  try {
    const fireHall = await resolveCityFireHall(picker.apiKey, city);
    await startDemo(picker.apiKey, fireHall, picker.quality, picker.cameraPreset);
  } catch (error) {
    document.body.classList.remove('city-picker-active', 'city-picker-launching');
    dom.cityPickerOverlay.hidden = true;
    dom.setupOverlay.hidden = false;
...
async function startDemo(apiKey, spawn, quality, cameraPreset) {
  if (!Cesium) throw new Error('CesiumJS did not load. Check your internet connection and reload.');
  state.station = { ...spawn, id: spawn.id || 'city', address: spawn.address || (spawn.name + ', Ontario'), headingDeg: Number(spawn.headingDeg) || 0 };
  ...
  if(turboButton) turboButton.hidden=false;
  state.lastFrame = performance.now();
  dom.cityPickerOverlay.hidden = true;
  document.body.classList.remove('city-picker-active', 'city-picker-launching');
  setStatus(dom.surfaceStatus, 'Surface: seeking 3D road', 'warn');
  // Dispatch sessions must not sample the station roof during startup. The authoritative
  // base spawn applies its road-derived floor lock immediately after startDemo returns.
  // Free Drive still uses normal photogrammetry surface alignment.
  if (!String(spawn.id||'').startsWith('fire-') && !String(spawn.id||'').startsWith('ems-')) {
    refreshSurfaceHeight();
    setTimeout(async()=>{for(let attempt=0;attempt<4;attempt++){if(await alignTruckToSurface({quiet:true}))break;await new Promise(r=>setTimeout(r,650));}},350);
  }
...
function setupJoystick() {
  ...
  dom.mobileRecenter.addEventListener('click', centerAndAlign);
...
function setupUi() {
  ...
  const savedCamera = readSetting('google3d.cameraPreset'); const initialCamera = savedCamera && CAMERA_PRESETS[savedCamera] ? savedCamera : 'birdsEye'; dom.cameraSelect.value = initialCamera; applyCameraPreset(initialCamera, { announce: false });
...
function applyKnownGroundHeight(height, { snap = false } = {}) {
  if (!Number.isFinite(height)) return false;
  const displayHeight = height + 0.15;
  state.surface.targetHeight = displayHeight;
  if (snap || !state.surface.locked || !Number.isFinite(state.surface.displayHeight)) state.surface.displayHeight = displayHeight;
  state.surface.locked = true;
  state.surface.mostDetailedTried = true;
  state.surface.lastAcceptedAt = performance.now();
  return true;
...
function activateDispatchGroundLock(lat, lon, height) {
  if (!Number.isFinite(height)) { state.dispatchGroundLock = null; return false; }
  state.dispatchGroundLock = { active: true, lat, lon, height };
  surfaceAlignment.reset();
  surfaceAlignment.accept(height, state.vehicle);
  surfaceAlignment.accept(height, state.vehicle);
  applyKnownGroundHeight(height, { snap: true });
  return true;
}

function currentDispatchGroundLockHeight() {
  const lock = state.dispatchGroundLock;
  if (!lock?.active || !Number.isFinite(lock.height)) return null;
  const moved = distanceMeters(lock.lat, lock.lon, state.vehicle.lat, state.vehicle.lon);
  const roadDistance = Number(state.roadMatch?.distance);
  // Keep the known station-floor elevation while the apparatus is still inside/near
  // the hall. Only hand control back to photogrammetry once it has actually driven
  // toward a mapped road, or has moved far enough to be safely clear of the building.
  if ((moved >= 15 && Number.isFinite(roadDistance) && roadDistance <= 6) || moved >= 65) {
    lock.active = false;
    surfaceAlignment.reset();
    state.surface.pending = false;
    state.surface.locked = false;
...
function closestPointOnRoadSegment(lat, lon, segment) {
  const cosLat = Math.cos(lat * Math.PI / 180);
  const ax = (segment.aLon - lon) * 111_320 * cosLat;
  const ay = (segment.aLat - lat) * 111_320;
  const bx = (segment.bLon - lon) * 111_320 * cosLat;
  const by = (segment.bLat - lat) * 111_320;
  const dx = bx - ax, dy = by - ay;
  const lenSq = dx * dx + dy * dy;
  const t = lenSq > 0 ? clamp(-(ax * dx + ay * dy) / lenSq, 0, 1) : 0;
  const east = ax + dx * t, north = ay + dy * t;
  return {
    lat: lat + north / 111_320,
    lon: lon + east / (111_320 * Math.max(0.2, cosLat)),
    heading: segment.heading,
  };
}

async function sampleCartographicHeights(points) {
  const scene = state.viewer?.scene;
  if (!scene?.sampleHeightSupported || !points.length) return [];
  const excluded = state.truckEntity ? [state.truckEntity] : [];
  let heights = points.map(point => scene.sampleHeight(point, excluded)).filter(Number.isFinite);
  if (heights.length < Math.min(5, Math.ceil(points.length / 3)) && typeof scene.sampleHeightMostDetailed === 'function') {
    try {
      const detailed = await scene.sampleHeightMostDetailed(points, excluded);
      heights = detailed.map(point => point?.height).filter(Number.isFinite);
    } catch {}
  }
  return heights;
}

async function sampleGroundHeightAroundSpawn(lat,lon) {
  const scene=state.viewer?.scene;
  if(!scene?.sampleHeightSupported) return null;

  // Prefer the nearest OSM road. Unlike the Google photogrammetry sample at the
  // station coordinate, the road centreline cannot be the station roof, so it is a
  // much stronger ground-elevation reference for an apparatus parked inside a hall.
  try { await roadDataPromise; } catch {}
  const road = state.roadMatcher?.match(lat, lon, state.vehicle.heading, 0);
  if (road && Number.isFinite(road.distance) && road.distance < 100) {
    const closest = closestPointOnRoadSegment(lat, lon, road);
    const roadSamples = [];
    for (const along of [-14,-7,0,7,14]) {
      for (const across of [-2.5,0,2.5]) {
        const north = Math.cos(closest.heading) * along + Math.cos(closest.heading + Math.PI / 2) * across;
        const east = Math.sin(closest.heading) * along + Math.sin(closest.heading + Math.PI / 2) * across;
        const pLat = closest.lat + north / 111_320;
        const pLon = closest.lon + east / (111_320 * Math.cos(closest.lat * Math.PI / 180));
        roadSamples.push(Cesium.Cartographic.fromDegrees(pLon,pLat,0));
      }
    }
    const roadHeights = (await sampleCartographicHeights(roadSamples)).sort((a,b)=>a-b);
    if (roadHeights.length >= 3) {
      // Lower-middle road samples reject cars and other raised photogrammetry clutter.
      const band = roadHeights.slice(0, Math.max(3, Math.ceil(roadHeights.length * 0.45)));
      return band[Math.floor(band.length / 2)];
    }
  }

  // Fallback: sample well outside a typical station footprint. The previous 14–30 m
  // rings could still land on the same large roof; these wider rings are intentionally
  // beyond the hall before taking a robust low-ground estimate.
  const samples=[];
  for(const radius of [40,60,85,110]) for(let i=0;i<16;i++){
    const a=i*Math.PI*2/16;
    const north=Math.cos(a)*radius,east=Math.sin(a)*radius;
    const pLat=lat+north/111320;
    const pLon=lon+east/(111320*Math.cos(lat*Math.PI/180));
    samples.push(Cesium.Cartographic.fromDegrees(pLon,pLat,0));
  }
  const heights=(await sampleCartographicHeights(samples)).sort((a,b)=>a-b);
  if(!heights.length) return null;
  const groundBand=heights.slice(0,Math.max(4,Math.ceil(heights.length*0.3)));
  return groundBand[Math.floor(groundBand.length/2)];
}
async function dispatchSpawnAtCoordinates(spawn) {
  const lat=Number(spawn?.lat),lon=Number(spawn?.lon),headingDeg=Number(spawn?.headingDeg);
  if(!Number.isFinite(lat)||!Number.isFinite(lon)||!Number.isFinite(headingDeg)) throw new Error('Invalid dispatch spawn.');
  if(!state.demoRunning||!state.viewer) throw new Error('3D driving runtime is not ready.');
  state.station={id:String(spawn.id||'dispatch-base'),name:String(spawn.name||'Dispatch base'),address:String(spawn.address||''),lat,lon,headingDeg};
  state.trainingRoadsEnabled=distanceMeters(lat,lon,PETERBOROUGH_CENTER.lat,PETERBOROUGH_CENTER.lon)<30000;
  resetVehicle({forceSurface:true}); dom.station.textContent=state.station.name; updateRoadMatch();
  state.viewer.camera.setView({destination:Cesium.Cartesian3.fromDegrees(lon,lat,340),orientation:{heading:state.vehicle.heading,pitch:currentPreset().pitch,roll:0}});
  const groundHeight=await sampleGroundHeightAroundSpawn(lat,lon);
  if(Number.isFinite(groundHeight)){
    state.station={...state.station,groundHeight};
    activateDispatchGroundLock(lat,lon,groundHeight);
  } else {
    state.dispatchGroundLock=null;
    refreshSurfaceHeight();
  }
  updateTruckEntity(); recenterCamera();
